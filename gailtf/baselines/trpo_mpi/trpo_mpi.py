...
def learn(args,
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, stochastic=True, visualize=visualize)
    ...
    while True:
        ...
        # surrogate_before = allmean(np.array(lossbefore))[0]
        g = allmean(g)
        if np.allclose(g, 0):
            ...
            for _ in range(10):
                ...
                logger.log("Evaluate changes...")
                meanlosses = surrogate_now, kl, *_ = allmean(np.array(compute_losses(*args)))
                improve = surrogate_now - surrogate_before
